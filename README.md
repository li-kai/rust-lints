...
| [`suggest_builder`](#suggest_builder) | warn | Structs with ≥ 6 named fields that don't derive `bon::Builder` |
...
Does not fire on structs that already derive `bon::Builder`, derive any trait in `skip_derives` (default: `Default`, `Queryable`, `Insertable`, `Selectable`), are named `*Builder`, have lifetime parameters, are `#[repr(C)]`, or are generated by macros. `PhantomData` fields are not counted toward the threshold.
