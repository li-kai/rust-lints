...
# Clippy group priority — highest severity first.  Groups not listed sort to
...
EXCLUDED_FIXABLE_LINTS = {
    ...
    "std_instead_of_alloc",  # broken without extern crate alloc
    "unseparated_literal_suffix",  # contradicts separated_literal_suffix (keep separated)
}
...
def _group_sort_key(group_name: str) -> tuple[int, str]:
    """Sort key that orders groups by CLIPPY_GROUP_ORDER, then alphabetically."""
    name = group_name.casefold()
    try:
        return (CLIPPY_GROUP_ORDER.index(name), name)
    except ValueError:
        return (len(CLIPPY_GROUP_ORDER), name)
...
def parse_fixable_lints(path: Path) -> OrderedDict[str, list[str]]:
    ...
    # Re-order by clippy group priority
    return OrderedDict(sorted(groups.items(), key=lambda kv: _group_sort_key(kv[0])))


def emit_lints(
    fixable_lints_path: Path | None = None, *, workspace: bool = False
) -> str:
    """Emit the recommended lint TOML sections.

    When *workspace* is True the headers become [workspace.lints.rust] and
    [workspace.lints.clippy] (for virtual-manifest workspaces).
    ...
    """
    prefix = "workspace." if workspace else ""
...
HOOK_HEADER_SHORT = "Pre-commit hook: auto-fix, format, check, banned crate detection."


def emit_hook(
    fixable_lints_path: Path | None = None, *, header: str = HOOK_HEADER_SHORT
) -> str:
...
CARGO_TOML_MARKER = "# Auto-generated by scripts/recommended-config.py"


def do_install(fixable_lints_path: Path | None = None) -> None:
    """Install pre-commit hook and patch Cargo.toml lint sections in-place."""
    root = Path.cwd()
    ...
    hook_header = HOOK_HEADER_SHORT
    if hook_path.exists() and "Auto-generated by scripts/recommended-config.py" in hook_path.read_text():
        hook_header = f"Auto-generated by scripts/recommended-config.py --format hook"
        if fixable_lints_path:
            hook_header += f" --fixable-lints {fixable_lints_path}"
    hook_path.write_text(emit_hook(fixable_lints_path, header=hook_header))
    hook_path.chmod(
    ...
    # Detect workspace virtual manifest
    workspace = any(l.startswith("[workspace]") for l in lines)

    cut = None
    had_marker = False
    lints_header = "[workspace.lints." if workspace else "[lints."
    for i, line in enumerate(lines):
        if line.startswith(CARGO_TOML_MARKER):
            had_marker = True
            cut = i
            break
        if line.startswith(lints_header):
            cut = i
            break
    if cut is None:
        # No lint sections yet — append after a blank line
        preamble = original.rstrip() + "\n\n"
    else:
        preamble = "".join(lines[:cut])

    lint_body = emit_lints(fixable_lints_path, workspace=workspace)
    if had_marker:
        args_str = f"--format lints"
        if fixable_lints_path:
            args_str += f" --fixable-lints {fixable_lints_path}"
        lint_body = f"{CARGO_TOML_MARKER} {args_str}\n\n{lint_body}"

    cargo_toml.write_text(preamble + lint_body + "\n")
    print(f"wrote {cargo_toml}")
...
def main() -> None:
    ...
    parser.add_argument(
        "--workspace",
        action="store_true",
        help="Emit [workspace.lints.*] headers (for virtual-manifest workspaces)",
    )
    parser.add_argument(
        "--install",
        action="store_true",
        help="Install pre-commit hook and patch Cargo.toml lint sections",
    )
    ...
    if args.format == "lints":
        print(emit_lints(args.fixable_lints, workspace=args.workspace))
