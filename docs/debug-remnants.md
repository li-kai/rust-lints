...
`LateLintPass::check_expr` — detect macro invocations for `println`, `print`, `eprintln`, `dbg`.

Follow the same `check_expr` + suppression zone pattern as `global_side_effect.rs`, but with macro detection instead of `DefId` resolution.

### Detection strategy

These are macros, not function calls. The expanded code calls internal functions (`std::io::_print`, etc.) that are unstable implementation details. Match on the macro itself:

1. Check `expr.span.from_expansion()` — if true, inspect the expansion source.
2. Get the macro's `ExpnData` via `span.ctxt().outer_expn_data()`.
3. Match the macro `DefId` against `sym::println`, `sym::print`, `sym::eprintln`, `sym::dbg`.
4. If matched, use the *call site* span (not the expansion span) for the diagnostic.

This avoids brittle string matching on expanded function paths.

### Suppression zones

Reuse `is_in_suppression_zone` from `global_side_effect.rs`, minus the entrypoint check:

1. `cx.sess().is_test_crate()` — skip test crates entirely.
2. `is_in_test(cx.tcx, expr.hir_id)` — skip `#[test]` functions.
3. Walk parent items for `#[cfg(test)]` modules.
4. Check for `#[allow(debug_remnants)]` — handled automatically by the lint infrastructure.

No implicit `fn main()` or binary crate exemptions.
...
Single `help` line with a concrete replacement example. The example uses the configured strategy (`tracing` or `log`). No multi-line notes or external URLs — keep the diagnostic actionable and parseable by agents.
...
pub struct DebugRemnantsConfig {
    pub suggested_strategy: String,  // "tracing" or "log"
    pub allow_in_tests: bool,
    pub allow_in_test_modules: bool,
}

impl Default for DebugRemnantsConfig {
    fn default() -> Self {
        Self {
            suggested_strategy: "tracing".to_string(),
            allow_in_tests: true,
            allow_in_test_modules: true,
        }
    }
